...
contract Main is Ownable, CloneFactory {
    ...
    event PutOnSaleEvent(bytes32 _key, uint8 royaltyFee, uint8 royaltyShare, address admin);
    ...
    function _makeHash(address user, address collectionId, uint256 tokenId) private pure returns (bytes32) {
    ...
    function generateImage() external payable {
    ...
    function putOnSale(address collectionId, uint256 tokenId, uint256 price) external payable {
        ...
        emit PutOnSaleEvent(_key, listInfo[_key].royaltyFee, listInfo[_key].royaltyShare, listInfo[_key].admin);
    }
    ...
    function auction(bytes32 _key, uint256 price) external payable {
        ...
        if (!isExist) {
            ...
            AuctionInfo memory auctionInfo = AuctionInfo({ taker: msg.sender, price: msg.value });
        ...
        if (price < oldValue) {
            (bool success, ) = payable(msg.sender).call{ value: oldValue - price }("");
            require(success, "Main: value transfer failed");
        }
    }
    ...
    function _exchangeDefaultNFT(bytes32 _key, uint price, address user, bool isBuyNow) private {
        ...
        if (listInfo[_key].admin != address(0) && 100 > listInfo[_key].royaltyShare) {
            ...
            (success, ) = payable(listInfo[_key].admin).call{ value: shareAmount }("");
            require(success, "Main: value transfer failed");
        }

        (success, ) = payable(treasury).call{ value: royaltyAmount - shareAmount }("");
        require(success, "Main: value transfer failed");
    ...
    function ListInfo(
        bytes32 _key
    ) external view returns (PutOnSaleInfo memory info, AuctionInfo[] memory auctionInfo, bool isValid) {
